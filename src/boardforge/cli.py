...
def build_parser() -> argparse.ArgumentParser:
    ...
    build_parser.add_argument("--target", default="all", help="Build target to pass to make")
    ...
    run_parser.add_argument("board", help="Board name or path")
    run_parser.add_argument("image", nargs="?", help="Firmware image path")
    run_parser.add_argument("--project-dir", help="Firmware project directory to build and infer the ELF from")
    run_parser.add_argument("--build", action="store_true", help="Build the firmware project before running")
    run_parser.add_argument("--target", default="all", help="Build target to pass to make when --build is used")
...
def _build(project_dir: str, target: str) -> int:
...
def _build_project(project_dir: Path, target: str) -> tuple[BuildResult, str, str] | int:
    ...
    artifacts = tuple(sorted(project_path.glob("*.elf")))
    return BuildResult(
...
def _resolve_run_image(image: str | None, project_dir: str | None, build: bool, target: str) -> tuple[Path, bool]:
    if build:
        if not project_dir:
...
def _run(engine, board_ref: str, image: str | None, project_dir: str | None, build: bool, target: str, timeout: float) -> int:
    try:
        resolved_image, inferred = _resolve_run_image(image, project_dir, build, target)
    except SystemExit as exc:
        return int(exc.code)

    if inferred and image is None:
        print(f"resolved-image: {resolved_image}")

    session = engine.load(board_ref, resolved_image)
    try:
        engine.run()
    except Exception as exc:
